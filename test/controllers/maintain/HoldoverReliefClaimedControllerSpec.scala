...
import pages.maintain.HoldoverReliefClaimedPage
...
class HoldoverReliefClaimedControllerSpec extends SpecBase {
  ...
  "HoldoverReliefClaimedController" must {

    "return OK and the correct view for a GET" in {

      val application = applicationBuilder(userAnswers = Some(emptyUserAnswers)).build()

      val request = FakeRequest(GET, holdoverReliefClaimedRoute)

      val result = route(application, request).value

      val view = application.injector.instanceOf[HoldoverReliefClaimedView]
    ...
    "populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = emptyUserAnswers.set(HoldoverReliefClaimedPage, validAnswer).success.value
    ...
    "redirect to the next page when valid data is submitted" in {
      ...
      val request = FakeRequest(POST, holdoverReliefClaimedRoute)
