...
import uk.gov.hmrc.maintaintrustdetailsfrontend.models.requests.{IdentifierRequest, OptionalDataRequest}
import uk.gov.hmrc.maintaintrustdetailsfrontend.repositories.{ActiveSessionRepository, PlaybackRepository}
...
class DataRetrievalActionImpl @Inject()(
  ...
  override protected def transform[A](request: IdentifierRequest[A]): Future[OptionalDataRequest[A]] = {
    ...
    def createdOptionalDataRequest(request: IdentifierRequest[A], userAnswers: Option[UserAnswers]): OptionalDataRequest[A] = {
      OptionalDataRequest(
        request.request,
        userAnswers,
        request.user
